...
def get_financial_loss_data(
    ...
    try:
        ...
        # ── YM возвраты ───────────────────────────────────────────────────
        # COG приоритет: wb_cogs по артикулу (точно) → ym_prices (приближённо) → 0
        ym_rows = db.execute(text("""
            SELECT
                r.return_id::text              AS srid,
                r.supplier_article             AS sku,
                r.defect_category              AS status,
                r.created_at                   AS created_dt,
                COALESCE(wc.cost_value, p.price, 0) AS cost_value,
                CASE
                    WHEN wc.cost_value IS NOT NULL THEN 'exact'
                    WHEN p.price IS NOT NULL THEN 'approximate'
                    ELSE 'none'
                END AS cost_type
            FROM ym_returns r
            ...
            LEFT JOIN (
                SELECT DISTINCT ON (TRIM(supplier_article))
                    TRIM(supplier_article) AS supplier_article,
                    cost_value
                FROM wb_cogs
                WHERE cost_value > 0
